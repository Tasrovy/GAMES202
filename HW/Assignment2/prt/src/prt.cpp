                    ...
                    for (int x = 0; x < width; x++)
                    {
                        ...
                        for (int l = 0; l <= SHOrder; l++)
                        {
                            for (int m = -l; m <= l; m++)
                            {
                                ...
                                SHCoeffiecents[l * (l + 1) + m] += Le * (float)sh_val * deltaW;
                            }
                        }
                    }
                }
            }
            for (auto& coeff : SHCoeffiecents)
            {
                coeff *= (4 * M_PI / sumWeight);
            }
    ...
    public:
        ...
        virtual void preprocess(const Scene* scene) override
        {
            ...
            if (m_Type == Type::Interreflection)
            {
                ...
                int sample_side = static_cast<int>(floor(sqrt(sample_count)));
                const int actual_samples = sample_side * sample_side;
                ...
                {
                    Eigen::MatrixXf nextBounceCoeffs = Eigen::MatrixXf::Zero(SHCoeffLength, mesh->getVertexCount());
                    for (int i = 0; i < mesh->getVertexCount(); i++)
                    {
                        const Point3f& v = mesh->getVertexPositions().col(i);
                        const Normal3f& n = mesh->getVertexNormals().col(i);
                        
                        for (int s = 0; s < sample_side; s++){
                            for (int p = 0; p < sample_side; p++){
                                double alpha = (s + rng(gen)) / sample_side;
                                ...
                                Intersection its;
                                Ray3f ray = Ray3f(v + (0.001 * n), wi);
                                if (!scene->rayIntersect(ray, its)) continue;
                                auto idx = its.tri_index;
                                auto bary = its.bary;

                                Eigen::VectorXf hitSH = 
                                    m_TransportSHCoeffs.col(idx.x()) * bary.x() +
                                    m_TransportSHCoeffs.col(idx.y()) * bary.y() +
                                    m_TransportSHCoeffs.col(idx.z()) * bary.z();
                                
                                nextBounceCoeffs.col(i) += hitSH * h;
                            }
                        }
                        nextBounceCoeffs.col(i)*=(4.0f  / (float)actual_samples);
                    }
                    m_TransportSHCoeffs +=nextBounceCoeffs;
                }
            }
        ...
        Color3f Li(const Scene* scene, Sampler* sampler, const Ray3f& ray) const
        {
            ...
            const Eigen::Matrix<Vector3f::Scalar, SHCoeffLength, 1>
                rL = m_LightCoeffs.row(0), gL = m_LightCoeffs.row(1), bL = m_LightCoeffs.row(2);
